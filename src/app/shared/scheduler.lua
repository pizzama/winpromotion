...
function M.registerSchedule(listener, interval, isPaused, isGlobal)
    if type(isPaused) ~= "boolean" then isPaused = false end
...
function M.performWithDelay( listener, time, isGlobal)
    local handle
...
--//////////////////////////////////////////////////////////////
---------------------------------------------------------------
function M.scheduleUpdateGlobal(listener)
    return sharedScheduler:scheduleScriptFunc(listener, 0, false)
end

function M.scheduleGlobal(listener, interval)
    return sharedScheduler:scheduleScriptFunc(listener, interval, false)
end

function M.unscheduleGlobal(handle)
    sharedScheduler:unscheduleScriptEntry(handle)
end

function M.performWithDelayGlobal(listener, time)
    local handle
    handle = sharedScheduler:scheduleScriptFunc(function()
