...
function BehaviorTree:ctor(data,customnames)
    self.id = b3.Com.createUUID()
    ...
    self:load(data,customnames)
end
-- 通过json配置表构建行为树
function BehaviorTree:load(data,customnames)
	...
	for id,nodeData in pairs(data.nodes) do
		-- print(k,v)
		local Cls = customnames[nodeData.name] or b3[nodeData.name]
...
function BehaviorTree:tick(target,blackboard)
    assert(target, 'target is important for tick method')
    assert(blackboard, 'balckboard is important for tick method')
