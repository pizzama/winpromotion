...
function WeightSelector:onOpen(tick)
	...
	tick.blackboard:set("runningChild",nil,tick.tree.id,self.id)
end
...
function WeightSelector:onTick(tick)
	...
	--优先选择上次正在执行中的节点
	local childIndex = tick.blackboard:get("runningChild",tick.tree.id,self.id)
...
function WeightSelector:executeChild(index,tick)
	...
	if status == b3.Com.RUNNING then
		tick.blackboard:set("runningChild",index,tick.tree.id,self.id)
	end
	return status
end
